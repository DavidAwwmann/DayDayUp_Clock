...
class GUI_Manager():
    ...
    def drawOptionBtns(self,mousex,mousey,mouseBtns):
        ...
        #         pygame.draw.rect(self.master,COLOR5,rect_btn,0,border_radius=b_radius)
        #         pygame.draw.rect(self.master,COLOR3,rect_btn,width*2,border_radius=b_radius)
        #         btnimg = self.getButtons(rawSize,0 if self.fullscreen else 16,64,COLOR3)
        #         self.master.blit(btnimg,rect_btn)
        #         rtn = 'rtns.switchFullscreen()'
        #     else:
        #         pygame.draw.rect(self.master,COLOR4,rect_btn,0,border_radius=b_radius)
        #         pygame.draw.rect(self.master,COLOR2,rect_btn,width*2,border_radius=b_radius)
        #         btnimg = self.getButtons(rawSize,0 if self.darkMode else 16,64,COLOR2)
        #         text_info = font_info.render('全屏模式(F11)',True,COLOR3,COLOR0)
        #         text_info.set_alpha(250)
        #         self.master.blit(text_info,(mousex+1,mousey-font_info.size('中')[1]-1))
        #         self.master.blit(btnimg,rect_btn)
        # else:
        #     pygame.draw.rect(self.master,COLOR1,rect_btn,width*2,border_radius=b_radius)
        #     btnimg = self.getButtons(rawSize,0 if self.darkMode else 16,64,COLOR1)
        #     self.master.blit(btnimg,rect_btn)
        # self.btnActiveRects['switchFullscreen'] = rect_btn
    ...
    def writeTimetableSchedules(self,timetable:List[List],weekday:int,nowSchedIdx:Union[int,None]=None):
        if self.compressLevel == 0:
            ...
            for schedule in timetable[weekday]:
                scheduleText = (f'{int(schedule[0]//100):02}:{int(schedule[0]%100):02} ~ {int(schedule[1]//100):02}:{int(schedule[1]%100):02}  ',f'{schedule[2]}')
            ...
            try:
                textNeedArea = (max(map(lambda a:a[0],map(font_timetable[0].size,map(lambda a:a[0]+'  '+a[1],timetableTexts)))),(font_timetable[0].size('文')[1]+int(self.screenSize[1]/180))*len(timetableTexts)-5)
            except ValueError:
                textNeedArea = (0,0)
