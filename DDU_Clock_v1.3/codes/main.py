...
# new_defined events: 
# TIMER.TIMEREVENT = pygame.USEREVENT+1, 
# GUI.IMPERATIVERESIZE = pygame.USEREVENT+2,
# INPUTS.DOUBLE_CLICK_TIMER_EVENT = pygame.USEREVENT+3
# REAL_QUIT = pygame.USEREVENT+4
...
if __name__ == '__main__':
    ...
    while shallLoop:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:   # observes QUIT event and do final initialize
                rtns.rereadSettings()
                gui.retrenchMode = False    # These two are INDISPENSABLE, otherwise pygame.quit() may not work properly!!
                real_quit_event = pygame.event.Event(REAL_QUIT,{})
                pygame.event.post(real_quit_event)
            elif event.type == REAL_QUIT:   # really quit
                shallLoop = 0       # No <break> anymore, otherwise pygame.quit() may not work properly!!
    ...
    sys.exit()      # Finally the horrible senior bug solved :D