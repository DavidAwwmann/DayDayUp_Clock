...
class RunDirectly():
    ...
    def main(self):
        ...
        self.mainwin.title('Timetable Editor')
        
        shouldExit = False
        while True:
            filepath = tkfile.askopenfilename(defaultextension='.txt',title='choose a <.txt> or <.json> file to open',initialdir='./',initialfile='timetable3.txt')
            if isFileOpenable(filepath,allowJson=True):
                break
            elif filepath == '':
                shouldExit = tkmsg.askyesno('Info','A file must be opened!\nDo you want to exit?')
                if shouldExit:
                    break
                else:
                    continue
            else:
                tkmsg.showerror('Error','Failed to open the file.')
        if shouldExit:
            exit()

        def checkExit():
            if self.et.notSavedYet:
                doFinalSave = tkmsg.askyesnocancel('Info','Progress unsaved!\nDo you want to save it?')
                if doFinalSave == None:
                    return None
                if doFinalSave:
                    ...
                    if rtn:
                        ...
                        tkmsg.showinfo('Info','File saved successfully')
                    else:
                        tkmsg.showerror('Error','Saving failed')
        ...
        self.mainwin.title(f'{"*" if self.et.notSavedYet else ""}{filepath} - Timetable Editor')
        ...
        fontSizeList = ['fontSize5','fontSize6','fontSize7','fontSize8','fontSize9','fontSize10','fontSize11','fontSize12','fontSize14','fontSize16','fontSize18','fontSize20','fontSize24']
        fontSizeVar.set('fontSize9')
        ...
        # objects in area1
        optionMenu = ttk.OptionMenu(area1,fontSizeVar,'fontSize9',*fontSizeList,command=optionMenuDo)
        optionMenu.grid(row=0,column=0,sticky='nesw')

        btnAdd = tk.Button(area1,text='Add Schedule..',font=('Courier New',10),command=self.main_showAddwin)
        btnAdd.grid(row=0,column=1,sticky='nesw')
        btnRemove = tk.Button(area1,text='Remove Schedule..',font=('Courier New',10),command=self.main_showRemovewin)
        btnRemove.grid(row=0,column=2,sticky='nesw')
        btnEdit = tk.Button(area1,text='Edit Schedule..',font=('Courier New',10),command=self.main_showEditwin_chooseSchedule)
        btnEdit.grid(row=0,column=3,sticky='nesw')

        btnUndo = tk.Button(area1,text='Undo',font=('Courier New',10),command=btnUndoDo)
        btnUndo.grid(row=0,column=4,sticky='nesw')
        btnRedo = tk.Button(area1,text='Redo',font=('Courier New',10),command=btnRedoDo)
        btnRedo.grid(row=0,column=5,sticky='nesw')

        # objects in area3
        btnSettings = tk.Button(area3,text='Modify Settings..',font=('Courier New',10),command=self.main_showSettingswin)
        btnSettings.grid(row=0,column=0,sticky='nesw',padx=[2,1],pady=[3,1])
        btnUndo = tk.Button(area3,text='File Options..',font=('Courier New',11),bg='#D0D0D0',fg='#3530A0',bd=3,command=self.main_showFileswin)
        btnUndo.grid(row=0,column=1,sticky='nesw',padx=[1,2],pady=[3,1])


        if self.et.filtered == []:
            tkmsg.showinfo('Info','File opened successfully.')
        else:
            tkmsg.showwarning('Info','Several format exceptions are fixed:\n'+'\n'.join(self.et.filtered))
    ...
    def main_showAddwin(self):
        ...
        tk.Label(addwin,text=' Add Schedule',font=('Courier New',10),fg='#202020').grid(row=0,column=0,columnspan=10,sticky='nsw')

        weekdayVar = tk.StringVar()
        weekdayNameList = ['Mon','Tue','Wed','Thu','Fri','Sat','Sun']
        weekdayVar.set('Mon')
        weekdayMenu = ttk.OptionMenu(addwin,weekdayVar,'Mon',*weekdayNameList)
        ...
        entry_schedName.insert(0,'Schedule Name')
            ...
            if nameMode == 1 and oldNameMode == 0:
                ...
                try:
                    if (oldName[0:2] == '"\'') and (oldName[-2:] == '\'"') and ("'" not in oldName[2:-2]):
                        ...
                        doModeSwitch = tkmsg.askokcancel('Warning','Evaluations have been used here.\nA forced switch may omit information!\nAre you sure to switch?')
                        if doModeSwitch:
                            entry_schedName.delete(0,'end')
        ...
        portInfo = '''Under <Advanced Input>, schedule name 
can be an eval()-able string.
Available variables:
  w — Weekday, 0 for Mon, 6 for Sun;
  y — Year, a four-digit integer;
  n — Month, 1 ~ 12;
  d — Date, 1 ~ 31;
  h — Hour, 0 ~ 23;
  m — Minute, 0 ~ 59;
  s — Second, 0 ~ 59;
  cd — Days between Jan-1st and today, 
       0 ~ 365;
  cw — Weeks between Jan-1st and today,
       0~52.'''
        portInfoVar.set('')
        ...
        inputModeList = ['Simple Input','Advanced Input']
        inputModeVar.set('Simple Input')
        oldNameMode = 0
        inputModeMenu = ttk.OptionMenu(addwin,inputModeVar,'Simple Input',*inputModeList,command=inputModeMenuDo)
        ...
        def btnConfirmDo(entryContents=None):
            ...
            if val == None:
                ...
                if val == 'syntax':
                    tkmsg.showerror('Error','Please check the inputs!')
                elif val == ('value','time'):
                    tkmsg.showerror('Error','Please check time formats!')
                elif val == ('value','name'):
                    tkmsg.showerror('Error','Please check schedule name formats!')
                elif val[0] == 'joint':
                    jointSchedTexts = []
                    ...
                    doCover = tkmsg.askokcancel('Warning',f'There have been schedules conflicting. Do you want to cover?\nConflicting schedules are:\n{jointSchedText}')
                    if doCover == True:
        ...
        btnConfirm = tk.Button(addwin,text='OK',font=('Courier New',10),width=12,command=btnConfirmDo)
        btnConfirm.grid(row=10,column=0,columnspan=4)
        btnCancel = tk.Button(addwin,text='Cancel',font=('Courier New',10),width=12,command=btnCancelDo)
    ...
    def main_showRemovewin(self):
        ...
        tk.Label(removewin,text=' Remove Schedule',font=('Courier New',10),fg='#202020').grid(row=0,column=0,columnspan=10,sticky='nsw')
        ...
        weekdayMenu = ttk.OptionMenu(removewin,weekdayVar,'Mon',*weekdayNameList,command=weekdayMenuDo)
        schedsMenu = ttk.OptionMenu(removewin,schedsVar,'',*schedsNameList)
        ...
        def btnConfirmDo():
            ...
            try:
                ...
                if self.et.removeSchedule(weekdayNameList.index(weekdayVar.get()),*rawSched):
                    ...
                    tkmsg.showerror('Error','Schedule not found.')
            except TypeError or ValueError:
                tkmsg.showerror('Error','Couldn\'t analyze the schedule.')
        def btnCancelDo():
            nonlocal removewin
            self.mainwin.columnconfigure(1,weight=0,minsize=0)
            removewin.destroy()
        ...
        btnConfirm = tk.Button(removewin,text='OK',font=('Courier New',10),width=12,command=btnConfirmDo)
        btnConfirm.grid(row=10,column=0,padx=[5,2],pady=10)
        btnCancel = tk.Button(removewin,text='Cancel',font=('Courier New',10),width=12,command=btnCancelDo)
    ...
    def main_showEditwin_chooseSchedule(self):
        ...
        tk.Label(editwin,text=' Edit Schedule',font=('Courier New',10),fg='#202020').grid(row=0,column=0,columnspan=10,sticky='nsw')
        tk.Label(editwin,text=' Choose a schedule to edit:',font=('Courier New',9),fg='#808080').grid(row=1,column=0,columnspan=10,sticky='nsw')
        ...
        weekdayMenu = ttk.OptionMenu(editwin,weekdayVar,'Mon',*weekdayNameList,command=weekdayMenuDo)
        schedsMenu = ttk.OptionMenu(editwin,schedsVar,'',*schedsNameList)
        ...
        btnConfirm = tk.Button(editwin,text='Next..',font=('Courier New',10),width=12,command=btnConfirmDo)
        btnConfirm.grid(row=10,column=0,padx=[5,2],pady=10)
        btnCancel = tk.Button(editwin,text='Cancel',font=('Courier New',10),width=12,command=btnCancelDo)
        btnCancel.grid(row=10,column=1,padx=[2,5],pady=10)
        editwin.mainloop()
    ...
    def main_showEditwin_inputNew(self,originSchedWeekday:Literal[0,1,2,3,4,5,6],originSchedText:str,originRawSched:Tuple[int,int,str],originNameMode:Literal[0,1]):
        ...
        weekdayNameList = ['Mon','Tue','Wed','Thu','Fri','Sat','Sun']
        tk.Label(editwin,text=' Edit Schedule',font=('Courier New',10),fg='#202020').grid(row=0,column=0,columnspan=10,sticky='nsw')
        tk.Label(editwin,text=' Please input the new schedule:',font=('Courier New',9),fg='#808080').grid(row=1,column=0,columnspan=10,sticky='nsw')
...
Available variables:
  ...
  cw — Weeks between Jan-1st and today,
       0~52.'''
        ...
        btnConfirm = tk.Button(editwin,text='OK',font=('Courier New',10),width=12,command=btnConfirmDo)
        btnConfirm.grid(row=10,column=0,columnspan=4)
        btnCancel = tk.Button(editwin,text='Cancel',font=('Courier New',10),width=12,command=btnCancelDo)
        btnCancel.grid(row=10,column=4,columnspan=4)
    ...
    def main_showFileswin(self):
        ...
        tk.Label(fileswin,text=' File Options',font=('Courier New',10),fg='#202020').grid(row=0,column=0,sticky='nsw')
        ...
        def btnRestoreDo():
            ...
            self.updateTextFrames()
            tkmsg.showinfo('Info','All changes are rollbacked, but still can recover using <Redo>.')
        def saveAsJson():
            btnCancelDo()
        ...
        btnSave = tk.Button(fileswin,text='Save',font=('Courier New',10),command=btnSaveDo)
        btnSave.grid(row=1,column=0,padx=5,pady=3,sticky='nesw')
        btnRestore = tk.Button(fileswin,text='Rollback',font=('Courier New',10),command=btnRestoreDo)
        btnRestore.grid(row=2,column=0,padx=5,pady=3,sticky='nesw')
        if self.et.isJson:
            btnSaveAs = tk.Button(fileswin,text='Export as TXT..',font=('Courier New',10),fg='#3540A0',command=saveAsTxt)
        else:
            btnSaveAs = tk.Button(fileswin,text='Export as JSON..',font=('Courier New',10),fg='#3540A0',command=saveAsJson)
        btnSaveAs.grid(row=3,column=0,padx=5,pady=3,sticky='nesw')
        btnExit = tk.Button(fileswin,text='Exit',font=('Courier New',10),fg='#C04000',command=btnExitDo)
        btnExit.grid(row=4,column=0,padx=5,pady=3,sticky='nesw')
        btnCancel = tk.Button(fileswin,text='Back to edit',font=('Courier New',10),command=btnCancelDo)
    ...
    def main_showTxtwin(self):
        ...
        tk.Label(txtwin,text=' Export as TXT..',font=('Courier New',10),fg='#303030').grid(row=0,column=0,columnspan=2,sticky='nsw')
        tk.Label(txtwin,text='Target version: ',font=('Courier New',10),fg='#151515').grid(row=1,column=0,sticky='nse')
        tk.Label(txtwin,text='timetable3 (for DDU_Clock_v1.3)',font=('Courier New',10),fg='#404040').grid(row=1,column=1,columnspan=3,sticky='nesw')
        ...
        def btnConfirmDo():
            savingText = f'{self.et.timetable!r}\n{self.et.settings!r}\n# Timetable version: 3'
            savingDir = tkfile.asksaveasfilename(defaultextension='.txt',title='Export as TXT..',initialdir='./',initialfile='timetable3')
            if savingDir == '':
                return None
            with open(savingDir,'w') as savingFile:
                try:
                    ...
                    tkmsg.showinfo('Info','File saved successfully.')
            btnCancelDo()
        btnConfirm = tk.Button(txtwin,text='Export..',font=('Courier New',10),command=btnConfirmDo)
        btnConfirm.grid(row=10,column=0,columnspan=2,padx=2,pady=5,sticky='nesw')
        btnCancel = tk.Button(txtwin,text='Cancel',font=('Courier New',10),command=btnCancelDo)
    ...
    def main_showJsonwin(self):
        ...
        tk.Label(jsonwin,text=' Export as <.json>',font=('Courier New',10),fg='#303030').grid(row=0,column=0,columnspan=2,sticky='nsw')
        tk.Label(jsonwin,text='Indent level: ',font=('Courier New',10),fg='#151515').grid(row=1,column=0,sticky='nse')
        def updatePreviewText(*args):
        ...
        targetIndentVar.set('partly(recommended)')
        targetIndentList = ['never','partly(recommended)','thoroughly']
        targetIndentMenu = ttk.OptionMenu(jsonwin,targetIndentVar,'partly(recommended)',*targetIndentList,command=updatePreviewText)
        ...
        def btnConfirmDo():
            ...
            savingDir = tkfile.asksaveasfilename(defaultextension='.json',title='Export as <.json>',initialdir='./',initialfile='timetable3')
            if savingDir == '':
                return None
            with open(savingDir,'w') as savingFile:
                try:
                    ...
                    tkmsg.showinfo('Info','File saved successfully.')
            btnCancelDo()
        btnConfirm = tk.Button(jsonwin,text='Export..',font=('Courier New',10),command=btnConfirmDo)
        btnConfirm.grid(row=10,column=0,columnspan=2,padx=2,pady=5,sticky='nesw')
        btnCancel = tk.Button(jsonwin,text='Cancel',font=('Courier New',10),command=btnCancelDo)
        btnCancel.grid(row=10,column=2,padx=2,pady=5,sticky='nesw')
    ...
    def main_showSettingswin(self):
        ...
        self.mainwin.columnconfigure(1,weight=8,minsize=450)  #column0 weights 10
        ...
        tk.Label(settwin,text=' Modify Settings',font=('Courier New',10),fg='#404040').grid(row=0,column=0,columnspan=10,sticky='nw')
        ...
        tk.Label(frameDarkMode,text='Dark mode:',font=('Courier New',10),fg='#161616').grid(row=0,column=0,sticky='nse')
        varDarkMode = tk.StringVar()
        varDarkMode.set('ON' if settingsDict['darkMode'] else 'OFF')
        ...
        tk.Label(frameCompressLevel,text='Brief GUI:',font=('Courier New',10),fg='#161616').grid(row=0,column=0,sticky='nse')
        varCompressLevel = tk.StringVar()
        varCompressLevel.set(f"{'Lv0' if settingsDict['compressLevel']==0 else 'Lv1' if settingsDict['compressLevel']==1 else 'Lv2'}")
        ...
        tk.Label(frameDoTimetableSearch,text='Schedule followed:',font=('Courier New',10),fg='#161616').grid(row=0,column=0,sticky='nse')
        varDoTimetableSearch = tk.StringVar()
        varDoTimetableSearch.set('Yes' if settingsDict['doTimetableSearch'] else 'No')
        ...
        tk.Label(frameScreenSize,text='Screen size (pixels): x=',font=('Courier New',10),fg='#161616').grid(row=0,column=0,sticky='nesw')
        ...
        tk.Label(frameTextFonts,text='Configure fonts:',font=('Courier New',10),fg='#161616').grid(row=0,column=0,columnspan=2,sticky='nsw')
        ...
        def btnBrowse1Do():
            path = tkfile.askopenfilename(defaultextension='.ttf',title='Choose a font file',initialdir='C:/Windows/Fonts/')
            if exists(path):
                varFont1.set(path)
        btnBrowse1 = tk.Button(frameTextFonts,text='Browse..',font=('Courier New',9),fg='#404070',bd=1,command=btnBrowse1Do)
        ...
        def btnBrowse2Do():
            ...
            if exists(path):
                varFont2.set(path)
        btnBrowse2 = tk.Button(frameTextFonts,text='Browse..',font=('Courier New',9),fg='#404070',bd=1,command=btnBrowse2Do)
        ...
        def btnBrowse3Do():
            ...
            if exists(path):
                varFont3.set(path)
        btnBrowse3 = tk.Button(frameTextFonts,text='Browse..',font=('Courier New',9),fg='#404070',bd=1,command=btnBrowse3Do)
        ...
        def btnBrowse4Do():
            ...
            if exists(path):
                varFont4.set(path)
        btnBrowse4 = tk.Button(frameTextFonts,text='Browse..',font=('Courier New',9),fg='#404070',bd=1,command=btnBrowse4Do)
        ...
        def btnBrowse5Do():
            ...
            if exists(path):
                varFont5.set(path)
        btnBrowse5 = tk.Button(frameTextFonts,text='Browse..',font=('Courier New',9),fg='#404070',bd=1,command=btnBrowse5Do)
        ...
        tk.Label(frameBgs,text='Bg Image:',font=('Courier New',10),fg='#161616').grid(row=0,column=0,sticky='nsw')
        ...
        varBg0 = tk.StringVar()
        varBg0.set('①Dark mode: Show' if settingsDict['doBackgroundImgDisplay'][0] else '①Dark mode: Hide')
        ...
        varBg1 = tk.StringVar()
        varBg1.set('②Bright mode: Show' if settingsDict['doBackgroundImgDisplay'][1] else '②Bright mode: Hide')
        ...
        tk.Label(frameBgs,text='①Path:',font=('Courier New',10),fg='#505050').grid(row=2,column=0,sticky='nse',pady=2)
        ...
        def btnBrowseBg0Do():
            path = tkfile.askopenfilename(defaultextension='.ttf',title='Choose an image file',initialdir=self.et.separate_dir_and_filename(varPath0.get())[0])
            if exists(path):
                varPath0.set(path)
        btnBrowseBg0 = tk.Button(frameBgs,text='Browse..',font=('Courier New',9),fg='#404070',bd=1,command=btnBrowseBg0Do)
        btnBrowseBg0.grid(row=2,column=2,sticky='nesw',padx=1.5,pady=2)

        tk.Label(frameBgs,text='②Path:',font=('Courier New',10),fg='#505050').grid(row=3,column=0,sticky='nse',pady=2)
        fgColor1 = colorsTextFonts[0]
        ...
        def btnBrowseBg1Do():
            path = tkfile.askopenfilename(defaultextension='.ttf',title='Choose an image file',initialdir=self.et.separate_dir_and_filename(varPath1.get())[0])
            if exists(path):
                varPath1.set(path)
        btnBrowseBg1 = tk.Button(frameBgs,text='Browse..',font=('Courier New',9),fg='#404070',bd=1,command=btnBrowseBg1Do)
        btnBrowseBg1.grid(row=3,column=2,sticky='nesw',padx=1.5,pady=2)

        tk.Label(frameBgs,text='Bg alpha:',font=('Courier New',9),fg='#505050').grid(row=4,column=0,sticky='nse',pady=2)
        ...
        tk.Label(frameLanguage,text='Default language:',font=('Courier New',10),fg='#161616').grid(row=0,column=0,padx=[5,0],sticky='nse')
        ...
        tk.Label(frameTimeOffset,text='Time offset: ',font=('Courier New',10),fg='#161616').grid(row=0,column=0,sticky='nesw')
        colorsTimeOffset = ['#204010','#F04030']
        fgColor = colorsTimeOffset[0]

        varTimeOffsetDirection = tk.StringVar()
        varTimeOffsetDirection.set('delay')
        def btnTimeOffsetDirectionDo():
            if varTimeOffsetDirection.get() == 'delay':
                varTimeOffsetDirection.set('ahead')
            else:
                varTimeOffsetDirection.set('delay')
        ...
        tk.Label(frameTimeOffset,text='day',font=('Courier New',9),fg='#161616').grid(row=0,column=4,sticky='nesw')
        ...
        tk.Label(frameTimeOffset,text='hour',font=('Courier New',9),fg='#161616').grid(row=0,column=6,sticky='nesw')
        ...
        tk.Label(frameTimeOffset,text='min',font=('Courier New',9),fg='#161616').grid(row=1,column=2,sticky='nesw')
        ...
        tk.Label(frameTimeOffset,text='sec',font=('Courier New',9),fg='#161616').grid(row=1,column=4,sticky='nesw')
        ...
        tk.Label(frameTimeOffset,text='ms ',font=('Courier New',9),fg='#161616').grid(row=1,column=6,sticky='nesw')
        ...
        tk.Label(frameColorfuncs,text='Fg color: Dark mode',font=('Courier New',9),fg='#161616').grid(row=0,column=0,padx=[5,0],sticky='nsw')
        tk.Label(frameColorfuncs,text='Bright mode',font=('Courier New',9),fg='#161616').grid(row=0,column=1,padx=[5,0],sticky='nesw')
        ...
        def btn00_do():
            nonlocal color00
            colorResult = tkcolor.askcolor(color00,title='Choose a color')
        ...
        def btn01_do():
            nonlocal color01
            colorResult = tkcolor.askcolor(color01,title='choose a color')
            if colorResult == (None,None):
                return None
            else:
        ...
        def btn10_do():
            nonlocal color10
            colorResult = tkcolor.askcolor(color10,title='choose a color')
            if colorResult == (None,None):
                return None
            else:
        ...
        def btn11_do():
            nonlocal color11
            colorResult = tkcolor.askcolor(color11,title='choose a color')
            if colorResult == (None,None):
                return None
            else:
        ...
        tk.Label(frameSidebarDisplayStyle,text='Schedule sheet style:',font=('Courier New',10),fg='#161616').grid(row=0,column=0,sticky='nse')
        varSidebarDisplayStyle = tk.StringVar()
        varSidebarDisplayStyle.set(f"{['Today-List','Week-List','Today-Columnar','Week-Columnar'][settingsDict['sidebarDisplayStyle']]}")
        ...
        tk.Label(frameColumnarGapMergeThreshold,text='Sched-Gap merging threshold (mins):',font=('Courier New',9),fg='#161616').grid(row=0,column=0,sticky='nesw')
        tk.Label(frameColumnarGapMergeThreshold,text='(1441=never)',font=('Courier New',8),fg='#161616').grid(row=0,column=2,sticky='nesw')
        colorsColumnarGapMergeThreshold = ['#204010','#F04030']
        ...
        tk.Label(frameRetrenchMode,text='Retrench mode:',font=('Courier New',10),fg='#161616').grid(row=0,column=0,sticky='nse')
        varRetrenchMode = tk.StringVar()
        varRetrenchMode.set('ON' if settingsDict['retrenchMode'] else 'OFF')
        ...
        tk.Label(frameMaxTps,text='Max tps:',font=('Courier New',10),fg='#161616').grid(row=0,column=0,sticky='nesw')
        colorsColumnarGapMergeThreshold = ['#204010','#F04030']
        ...
        def btnConfirmDo():
            if (1 in invalidCode) or not((int(bgAlphaSpinbox.get()) in range(101)) and 
                ...
                tkmsg.showerror('Error','Syntax error occurred in settings.')
            else:
                    ...
                    for colorfunc in darkmodeColorfuncs:
                        for x in range(0,256):
                            try:
                                ...
                                tkmsg.showerror('Error',f"Error occurred at x={x} in expression {colorfunc} of <Foreground color>.")
                                return None
                            else:
                                if n not in range(0,256):
                                    tkmsg.showerror('Error',f"Y not in 0~255 at x={x} in expression {colorfunc} of <Foreground color>.")
                                    return None
                ...
                k = 1 if varTimeOffsetDirection.get()=='delay' else -1
        ...
        pageUpdnVar.set('Next Page..')
        def pageUpdnDo():
            if pageUpdnVar.get() == 'Next Page..':
                settPage0.grid_remove()
                settPage1.grid()
                pageUpdnVar.set('Prev Page..')
            else:
                settPage1.grid_remove()
                settPage0.grid()
                pageUpdnVar.set('Next Page..')
        btnConfirm = tk.Button(frameFinal,text='OK',font=('Courier New',11),bd=3,fg='#089000',command=btnConfirmDo)
        btnConfirm.grid(row=0,column=0,padx=2,pady=5,sticky='nesw')
        btnCancel = tk.Button(frameFinal,text='Cancel',font=('Courier New',11),bd=3,fg='#C04000',command=btnCancelDo)
    ...
    def updateTextFrames(self):
        ...
        self.mainwin.title(f'{"*" if self.et.notSavedYet else ""}{self.filepath} - Timetable Editor')
        texts = self.weekdayScheduleFormat(self.timetable)
        for frameIdx in range(7):
            frame = self.textFrames[frameIdx]
    ...
    def weekdayScheduleFormat(self,timetable:list,do_scheduleText_break_into_2_lines:bool=True) -> list:
        ...
        for i in range(7):
            finalText[i] = f' {weekdayNames[i]}: \n'+finalText[i]
        return finalText

    @deco
    def main_textToTuple(self,text:str) -> Tuple[int,int,str,Literal[0,1]]:
        '''text example:   '09:30~10:15 SchedName'
        output example: [930,1015,"'SchedName'"]
